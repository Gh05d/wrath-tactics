...
- **Text drawn directly on the book-page art: use `UIHelpers.AddPageLabel`, not `AddLabel`.** Same signature as `AddLabel` — it just adds the black outline before returning. Plain white text on the parchment background is unreadable; this shipped as a defect once (titles, empty-state text, status lines, and list headers in `PackPanel`/`PresetPanel` had no background under them) and was fixed by wrapping the ad-hoc outline pattern that `TacticsPanel` already used in a couple of places into this helper. Leave `AddLabel` alone for anything that already sits on its own dark background (buttons, chips, swatches, rows) — the outline there is pure rendering cost with no readability gain.
