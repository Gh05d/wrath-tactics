...
- **`UsableItemType` enum (5 values)**: `Other=0 / Wand=1 / Scroll=2 / Potion=3 / Utility=4`. Rods and special-power devices (Devouring Lust, Skeletal Finger, Rod-of-X actives) use `Utility` — there is NO `BlueprintItemEquipmentRod` class. Metamagic rods are also `Utility` but additionally carry `MetamagicRodMechanics`. Inventory scans must include `Utility` to surface rod-only abilities (see four-pass note below).
