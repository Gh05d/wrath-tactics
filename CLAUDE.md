...
Pure-logic xUnit suite in `WrathTactics.Tests/` (net481). Covers `ConditionEvaluator.CompareCount`, `ConditionEvaluator.ResolveCountThreshold`, `BuffBlueprintProvider.IsCrusadeOnlyBuff`, `CommonBuffRegistry.IsEnemySubject` / `GetDefaultGuids`, `RangeBrackets.MaxMeters`. Mono hosts the .NET-Framework runner on Linux; no Game.Instance / Unity needed.
...
**Nexus-upload action fails with Cloudflare 504**: transient Nexus-side timeout during the multipart part upload, not a workflow/config problem. Fix: `gh run rerun <run-id> --repo Gh05d/wrath-tactics --failed` — don't debug the workflow (observed + resolved this way on v1.18.0). Upload-Ausgang abwarten statt `gh run list` zu pollen: `gh run watch <run-id> --repo Gh05d/wrath-tactics --exit-status`.
