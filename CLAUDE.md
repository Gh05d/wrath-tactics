...
- **`IsHealingSpell` uses substring keyword matching** (not component inspection): recognizes `cure`, `heal`, `lay on hands`, `channel positive`, plus German `wunden`/`heilung`. Known imprecision — `cure` still matches Cure Disease / Cure Deafness / Neutralize Poison; `heal` could match Heal Mount. UMD-gate (for scrolls) limits real-world mis-casts; the 1.2.0 fix dropped `restoration` (Restoration removes ability damage / drain / negative levels, not HP — user-reported: "HEAL order uses Restoration scrolls as if they were HP heals"). Before adding a new keyword, verify it matches ONLY HP-heals. Correct future fix: component-based detection (`ContextActionHealTarget` in the ability's action graph), flagged in `ActionValidator.MatchesHealKeyword`.
