...
| `Localization/*.json` (5 locales) | Modify | New keys: `cast.rod.label` ("Rod"), `cast.rod.none` ("(none)"), `cast.rod.tooltip` ("Rod must sit in the caster's quickslot. Mod activates it before the cast; engine applies metamagic and spends one charge. Falls back to normal cast silently if rod is unavailable, has no charges, or the spell isn't eligible."), and ten `cast.rod.<metamagic>` labels (`cast.rod.quicken`, `cast.rod.empower`, …) for the dropdown options. |
