...
`MoveToTarget` (new `ActionType`, appended): walk towards the resolved unit or point until inside `ActionDef.MoveWithin` (a `RangeBracket`, default Melee). Engine command `UnitMoveTo(destination, approachRadius = MaxMeters(bracket), distanceXZ: true)` in the Move slot via `RunVerified`; validator requires `State.CanMove` and `RangeBrackets.Beyond(distance, bracket)`, so the rule goes quiet once in range and follows a moving target while it is not. Approach-only by decision (no kiting in 1.30). It passes `HasCrossSlotConflict` like an animated command because `Run(Move)` removes an own Standard command; over the engine's auto-attack it fires, which is the player-click semantics. `UnitMoveTo` never sets `IsActed`, so the cooldown refund counts `Result == Success` as acted.
