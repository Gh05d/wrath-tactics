...
## Post-release amendment (1.29.1, 2026-09-08)

The shipped design was wrong in one load-bearing sentence: *"Under slot gating the occupied slot no longer blocks unrelated rules, so buffering is no longer harmful."* Buffering is harmless; **starting** is not. The engine starts each unstarted command the moment its own cooldown allows, ticking slots in array order, and it never checks whether another animated command is already running (its single cross-slot hold is Standard-waits-while-Move-`IsRunning`). Two `UnitUseAbility`/`UnitAttack` commands running at once share one `AnimationManager`; `AnimationManager.Execute` releases the running action and `UnitCommand.Tick` interrupts the command whose animation "is done but not acted". Net effect of the reporter's own rule list on 1.29.0: cast and cackle issued in one tick started in the same frame, the cast died every time, the standard action was never spent, and only the rule cooldown was burnt. Confirmed in the Owlcat `GameLog.txt` of the 1.29.0 smoke test itself (Ember: heal + cackle in one tick ⇒ `CastSpell … not acted`), which the mod log could not show because `EXECUTED` records issuance, not effect.

Fix: §4's guard is generalised from "own slot holds an unfinished `UnitUseAbility`" to a cross-slot check for every command-issuing rule (`ActionSlots.CheckConflict`, pure and tested; `TacticsEvaluator.HasCrossSlotConflict`). A started animated command in any other slot blocks. A *pending* one blocks too, with two exceptions that are safe by engine rule: Move over a pending Standard whose `Cooldown.StandardAction` exceeds 0.25 s while the Move slot is free (Move starts this frame, engine then holds Standard), and Swift over a pending Standard with more than 2.5 s left (nothing holds Standard back from a running Swift, so the cooldown must outlast the swift animation). The per-tick budget, the priority gate and `ActiveRuleTracker` are unchanged. The reporter's list now resolves as: tick A cast; tick B attack (buffered on cooldown) + cackle (allowed, runs ahead of the attack); all three per round, no collision. `PlayerCommandGuard` additionally logs each finished own command's `UnitCommand.Result`, so a future "fires but never lands" is visible in the mod log as `ended: Interrupt`.

Second deck test (2026-09-08, 20:22 session) found the remaining hole: the guard's Move-over-pending-Standard exception reasoned about the *cooldown* only. A pending Standard can also be unstarted because its executor is still walking into range, and `UnitCommands.Run(cmd)` (IL-verified) interrupts every other unstarted command with `!IsUnitCloseEnough()` whenever `cmd` is a Standard or Move command. Cackle issued over an approaching Evil Eye therefore cancelled the Evil Eye, emptied the Standard slot, and `AiBrainController.TickBrain` — which only stands down while the unit holds a command with `AiCanInterrupt == false` — filled it with Ember's default action (`BlueprintAiAttack` → `Brain.AutoUseAbility`, Ray of Frost). Log signature: `own Evil Eye [Standard] ended: Interrupt` in 8 of 9 rounds, each followed by `Ray of Frost running in Standard`. `CheckConflict` now takes `occupantApproaching` (`!IsStarted && !IsUnitCloseEnough()`) and returns `SlotConflict.Approaching` ahead of any cooldown reasoning; the skip line reads `unit busy — X in Standard still approaching its target`. Side finding: `Commands.Run discarded … (engine veto)` while a foreign cast runs is the engine *queueing* our command (`TryAddToQueueInsteadOfRunImmediately`), not vetoing it. 1.29.1 adopts the engine's semantics: a queued own command counts as issued and is tracked; both trackers use `Commands.ContainsOrQueued` for residency so a queue wiped by a later `Run()` self-heals. The same queue is where a player click lands while the unit is busy, and a slot-only foreign check could not see it — `PlayerCommandGuard` now scans the queue too. A `CommandDiagnostics` handler (EventBus start/act/end + Harmony prefix on `UnitCommand.Interrupt`) records who ends which command, frame-accurately. Details in `gotchas-engine.md`.

Third deck test (2026-09-09, 11:31 session) with that diagnostics build settled the remaining Evil-Eye losses and overturned the design's core premise. Every "allowed overlap" of Cackle over a pending Evil Eye ended with `own Evil Eye interrupted … by UnitCommands.InterruptAndRemoveCommand <- InterruptAndRemoveCommand <- InterruptAndRemoveCommand <- UnitCommands.Run` — three nested frames, i.e. `Run(Move)` → `InterruptAndRemoveCommand(Move)` → `InterruptAndRemoveCommand(Move, alsoPaired: true)` → `InterruptAndRemoveCommand(Standard, false)`. **Move and Standard are paired slots: issuing either removes the other, pending or running.** §2's "slots are independent, only Standard is gated" holds for Swift and Free only. `CheckConflict` therefore drops the cooldown-margin exception entirely: Move over an own Standard command is `PairedOwn` (blocked); Move over an engine-issued Standard command (auto-attack, default action) is allowed, because cancelling it is what the player's own click does and the party AI re-issues it. The per-round shape for a hex witch is now strictly serial — cast tick, cackle tick, attack tick — which is also what the engine enforces for a human player. The remaining `own Evil Eye interrupted (started=True) by UnitCommandController.TickCommand` cases (two in the session) come from one of TickCommand's three interrupt sites (`ShouldBeInterrupted`, `InterruptAsSoonAsPossible && IsInterruptible`, `!CanAct`/`!CanCast`); the diagnostics now dump those inputs on every interrupt. Fourth and fifth deck test (same day): with the paired-slot rule in place every remaining interrupt of an own command was attributable — player clicks (`ClickWithSelectedAbilityHandler.OnClick`), global-over-character preemption by design, dead targets (`shouldBeInterrupted=True`), the unit going prone or down (`UnitProneController`, `UnitLifeController.SetLifeState`). The fourth test also exposed that our own Move rule's `Run()` wiped our own queued cast (7× "vanished"); since then any own command in `Commands.Queue` blocks every command-issuing rule for the tick, and a command that still vanishes unstarted refunds its rule cooldown. Zero `not acted` in the Owlcat log for the final round.

## 1.30 amendment (2026-09-15): engine action budget + Move to target

The reporter's next observation was correct and generalised: with paired slots, a cooldown-0 Standard rule above a Move rule re-issues immediately after its cast, the new command buffers in Standard for the whole action cooldown, and the Move rule sees `PairedOwn` on every tick. Reordering only moves the starvation to the other side; with several Standard and several Move rules no order or cooldown yields "one Standard, then one Move per round". The engine already keeps that budget: RTWP `SpendAction` books a standard action as `StandardAction = 6 − t` plus `MoveAction = 3`, a move as `MoveAction += 3`, a swift as `SwiftAction = 6`, and `HasCooldownForCommand(type)` is its verdict. 1.30 asks it before issuing (`ActionSlots.ActionSpent`, pure): a rule whose slot is spent and whose remaining cooldown exceeds the tick interval is skipped without stamping its cooldown; shorter remainders still buffer so the command starts the moment the engine allows. Net effect: within a slot type the list order is the priority, across slot types every available action gets filled each round — the original intent of this spec, now with the engine's numbers instead of our own. Cost: up to one tick of latency per action on a 3 s tick (cast every 9 s with Cackle in between instead of 6 s without); a shorter tick interval is the cheap lever, queueing our own commands behind our own paired command via the public `UnitCommands.AddToQueue` would shave the rest and is deferred until the difference shows in practice.

The BlueprintCore `HasActionsAvailable` condition the reporter pointed at is not an engine type (it lives in BlueprintCore's `Conditions.New`, absent from all 127 game DLLs); it wraps the same engine methods (`UnitEntityData.HasStandardAction()/HasMoveAction()/HasSwiftAction()`), which is why the idea transfers directly.

`MoveToTarget` (new `ActionType`, appended): walk towards the resolved unit or point until inside `ActionDef.MoveWithin` (a `RangeBracket`, default Melee). Engine command `UnitMoveTo(destination, approachRadius = MaxMeters(bracket), distanceXZ: true)` in the Move slot via `RunVerified`; validator requires `State.CanMove` and `RangeBrackets.Beyond(distance, bracket)`, so the rule goes quiet once in range and follows a moving target while it is not. Approach-only by decision (no kiting in 1.30). It passes `HasCrossSlotConflict` like an animated command because `Run(Move)` removes an own Standard command; over the engine's auto-attack or a pending default-action cast it fires, which is the player-click semantics. First 1.30 deck round corrected one edge: over a foreign cast that has already started, `Run(Move)` queues behind the (uninterruptible) cast and flags it `InterruptAsSoonAsPossible`, which kills it in its wind-up — 10 of 10 Cackles took out the AI's Ray of Frost. Move rules now wait for a running foreign cast; the budget gate hands them the following tick. `UnitMoveTo` never sets `IsActed`, so the cooldown refund counts `Result == Success` as acted.
