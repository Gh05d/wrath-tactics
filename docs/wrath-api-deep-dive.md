...
**Mod integration (1.22.0+)**: picker (`SpellDropdownProvider.EmitConversionEntries`) runs a conversion pass AFTER all static enumeration, filtered against the set of PASS-1 blueprint GUIDs (known/custom/special spells + variants) — this drops the variant re-inclusions and spontaneous-cure noise, leaving conversion-only content. The filter is a Contains-check against the pass-1 set, NOT a running dedup: conversion entries must not consume the GUID, or the first parent hides all later ones — metamagic-prepared parents are distinct casts and each emits its own conversion entry (base Fireball L3 and Heightened Fireball L10 both list Cluster Bomb; 1.22.0 shipped with `Add` and hid the metamagic entries, fixed in 1.22.1). Exact duplicates die on the key-level `seen` set (keys carry `@L` + `#mask`). Same-blueprint conversions bypass that filter and get the `~A<actionType>` key segment as discriminator (`guid[@L<n>][>V<guid>][~A<n>][#<mask>]`). Resolution: `ActionValidator.FindConversion` as fallback after `FindVariantBlueprint` missed; a same-blueprint key without `~A` is deliberately unresolvable (ambiguous with the parent). Cast side needs nothing: conversions flow through `UnitUseAbility.CreateCastCommand` with the `Rulebook.Trigger` fallback, and the slot probe already uses `ConvertedFrom ?? ability`.
